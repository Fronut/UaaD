...
| 任务 | 验收标准 | 完成状态 |
|------|----------|----------|
| 数据库行数 ≥ 1 万条 | 各主表总行数超过 10,000 | ✅ **17,824 行** |
| JMeter 并发压测 | 1000 线程，错误率 0%，关键 API < 200ms | ✅ **P90 = 27ms，错误率 0%** |
...
| 表 | 行数 | 来源 |
|---|---|---|
| users | 1,006 | seeder 1,000 批量用户 + 5 固定测试账号 + 1 JMeter 新注册 |
| activities | 106 | seeder 100 条 + JMeter 预飞创建 3 条 + 手动测试 3 条 |
| enrollments | 7,004 | seeder 6,000 + JMeter 压测 999 + 手动测试 4 + 其他 1 |
| orders | 3,164 | seeder 2,160 + Kafka Worker 异步处理生成 1,004 |
| notifications | 1,544 | seeder 540 + 报名/订单事件自动触发 1,004 |
| user_behaviors | 5,000 | seeder 批量注入 |
| **合计** | **17,824** | **远超 10,000 条要求** |
...
- 1,001 条 JMeter 订单（`amount=0`，免费活动）因 15 分钟支付超时自动关闭，状态为 `CLOSED`，属正常业务逻辑
...
| Ramp-up 时间 | 30s |
| 每线程请求数 | 1 |
| 目标活动 | loadtest-20260523-200228（id=106，capacity=5,000，Redis 已热身） |
| CSV 数据 | 1,000 条独立 JWT token + activity_id（由 `gen_jmeter_data` 生成） |
...
| 平均响应时间 | 25ms |
| P50 | 20ms |
| P90 | **27ms**（远低于 200ms 要求） |
| P99 | 120ms |
| 最大响应时间 | 181ms |

### 结论

在 17,000+ 行数据背景下，报名接口 1,000 并发线程压测零错误，P90 = 27ms，满足"关键 API 响应 < 200ms"的验收要求。Redis Lua 原子扣减 + Kafka 异步落库的架构在本机单节点环境下表现稳定，无超卖，无请求丢失。
...
- JMeter 订单全部 `CLOSED`：测试活动价格设为 0，Kafka worker 正常创建 1,001 条 `PENDING` 订单，15 分钟超时后由 OrderExpiry 定时任务自动关闭，符合设计预期，非 Bug。
