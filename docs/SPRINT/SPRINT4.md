...
在 **27,000+** 行数据（含万级压测新增数据）背景下，**10,000 个独立用户**并发报名，全程零错误。持续吞吐量稳定在 166 req/s，P50 = 26ms，P90 = 142ms，均满足"关键 API 响应 < 200ms"验收标准。T+0s 的 759 req/s 瞬时峰值证明 Redis Lua 原子扣减可抵抗突发流量冲击，无超卖，无请求丢失。
